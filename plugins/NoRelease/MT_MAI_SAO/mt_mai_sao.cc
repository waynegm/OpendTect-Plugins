...
mAttrDefCreateInstance(MT_MAI_SAO)

void MT_MAI_SAO::initClass()
{
    mAttrStartInitClass
...
void MT_MAI_SAO::updateDefaults( Desc& desc )
{
...
MT_MAI_SAO::MT_MAI_SAO( Desc& desc )
: Provider( desc )
{
    if ( !isOK() ) return;
...
void MT_MAI_SAO::prepPriorToBoundsCalc()
{
    sampgate_ = Interval<int>( mNINT32(gate_.start/refstep_), mNINT32(gate_.stop/refstep_) );
    Provider::prepPriorToBoundsCalc();
}

const BinID* MT_MAI_SAO::desStepout( int inp, int outp ) const
{
    return inp ? 0: &stepout_;
}

const Interval<int>* MT_MAI_SAO::desZSampMargin( int inp, int outp ) const
{
    return inp ? 0: &sampgate_;
}

bool MT_MAI_SAO::getInputData( const BinID& relpos, int zintv )
{
    input_1_.erase();
    input_2_.erase();
...
bool MT_MAI_SAO::computeData( const DataHolder& output, const BinID& relpos, int z0, int nrsamples, int threadid) const
{
...
void MT_MAI_SAO::computeOutput( const Array2DImpl<float>& inp1, 
