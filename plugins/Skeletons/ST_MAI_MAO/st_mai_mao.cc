...
mAttrDefCreateInstance(ST_MAI_MAO)

void ST_MAI_MAO::initClass()
{
    mAttrStartInitClass
...
void ST_MAI_MAO::updateDefaults( Desc& desc )
{
...
ST_MAI_MAO::ST_MAI_MAO( Desc& desc )
: Provider( desc )
{
    if ( !isOK() ) return;
...
void ST_MAI_MAO::prepPriorToBoundsCalc()
{
    sampgate_ = Interval<int>( mNINT32(gate_.start/refstep_), mNINT32(gate_.stop/refstep_) );
    Provider::prepPriorToBoundsCalc();
}

const Interval<int>* ST_MAI_MAO::desZSampMargin( int inp, int outp ) const
{
    return inp ? 0: &sampgate_;
}

bool ST_MAI_MAO::getInputData( const BinID& relpos, int zintv )
{
...
bool ST_MAI_MAO::computeData( const DataHolder& output, const BinID& relpos, int z0, int nrsamples, int threadid) const
{
...
void ST_MAI_MAO::computeOutput_1( const Array1DImpl<float>& inp1, 
...
void ST_MAI_MAO::computeOutput_2( const Array1DImpl<float>& inp1, 
                                  const Array1DImpl<float>& inp2,
                                  const Array1DImpl<float>& inp3,
                                  Array1DImpl<float>& result ) const
{
    ...
    for (int idx=0; idx<sz; idx++) {
        result.set(idx, inp3.get(idx));
    }
}    

void ST_MAI_MAO::computeOutput_3( const Array1DImpl<float>& inp1, 
                                  const Array1DImpl<float>& inp2,
                                  const Array1DImpl<float>& inp3,
                                  Array1DImpl<float>& result ) const
{
